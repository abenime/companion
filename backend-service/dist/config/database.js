    ...
    async mockQuery(text, params) {
        ...
        try {
            ...
            if (sql.match(/FROM\s+users\s+WHERE\s+email/i)) {
                ...
                rows = this.mockData.users.filter((u) => u.email === email).map((u) => ({ id: u.id }));
            }
            ...
            // 6. SELECT u.id, u.name, u.email, u.password_hash ... (Join User, Profile, Connections on Email)
                ...
                if (user) {
                    const profile = this.mockData.user_profiles.find((p) => p.user_id === user.id) || {
                        age: 25,
                        gender: 'Female',
                        work_status: 'full-time'
                    };
                    const connection = this.mockData.user_connections.find((c) => c.user_id === user.id) || {
            ...
            // 8. UPDATE user_connections SET ... WHERE user_id = $1
                ...
                const user_id = params[0];
                let conn = this.mockData.user_connections.find((c) => c.user_id === user_id);
                if (!conn) {
            ...
            // 9. SELECT * FROM user_baselines WHERE user_id = $1
                ...
                const user_id = params[0];
                const baseline = this.mockData.user_baselines.find((b) => b.user_id === user_id) || {
                    user_id,
                    avg_sleep_hours: 7.5,
                    avg_steps: 8000,
                    avg_screen_time_mins: 240,
            ...
            // 10. SELECT * FROM daily_features WHERE user_id = $1 AND measured_date = $2
                ...
                rows = this.mockData.daily_features.filter((f) => {
            ...
            // 11. INSERT INTO daily_features ON CONFLICT
                ...
                let feature = this.mockData.daily_features.find((f) => {
                    return f.user_id === user_id && new Date(f.measured_date).toISOString().split('T')[0] === dateStr;
                });
                const featureObj = {
            ...
            // 12. SELECT inference_date FROM ai_inferences ... (Timeline)
                ...
                const user_id = params[0];
                rows = this.mockData.ai_inferences.filter((i) => i.user_id === user_id);
            ...
            // 14. SELECT SUM(CASE WHEN signal_type = 'steps' ... FROM raw_signals
                ...
                const user_id = params[0];
                const dateStr = new Date(params[1]).toISOString().split('T')[0];
                const signals = this.mockData.raw_signals.filter((s) => {
                    return s.user_id === user_id && new Date(s.timestamp).toISOString().split('T')[0] === dateStr;
                });
                const total_steps = signals.filter((s) => s.signal_type === 'steps').reduce((sum, s) => sum + Number(s.numeric_value), 0);
                const sleep_duration = signals.filter((s) => s.signal_type === 'sleep').reduce((max, s) => Math.max(max, Number(s.numeric_value)), 0);
                const total_screen_time = signals.filter((s) => s.signal_type === 'screen_time').reduce((sum, s) => sum + Number(s.numeric_value), 0);
                const total_context_switches = signals.filter((s) => s.signal_type === 'context_switches').reduce((sum, s) => sum + Number(s.numeric_value), 0);
                const social_time = signals.filter((s) => s.signal_type === 'social_app_time').reduce((sum, s) => sum + Number(s.numeric_value), 0);
                const productivity_time = signals.filter((s) => s.signal_type === 'productivity_app_time').reduce((sum, s) => sum + Number(s.numeric_value), 0);
                const typing_cadence = signals.filter((s) => s.signal_type === 'typing_cadence');
                const avg_typing_cadence = typing_cadence.length > 0 ? typing_cadence.reduce((sum, s) => sum + Number(s.numeric_value), 0) / typing_cadence.length : 0;
                rows = [{
                        total_steps,
                        sleep_duration,
                        total_screen_time,
