...
export interface VarianceFeatures {
    sleepDeficitPercent: number;        // negative means sleep loss
    stepDeficitPercent: number;         // negative means sedentary
    screenTimeDeltaPercent: number;     // positive means screen overload
    contextSwitchDensity: number;       // context switches per hour
    socialRatio: number;                // social app time relative to productivity time
    keystrokeSpeedDeltaPercent: number; // typing cadence variance
}
...
export class FeatureExtractionService {
    ...
    public static async recalculateUserBaseline(userId: string): Promise<void> {
        const db = DatabaseConnection.getInstance();
